...
func HandleTab(ktr *etree.Element, handle_str string, mysqlConn *gorm.DB, gpConn *gorm.DB) string {
	...
	step_in.FindElement("sql").SetText(fmt.Sprintf("select * from %s where update_time > ${etl_update_time}", mysql_tab_infos[1]))
	step_in_name := mysql_tab_infos[1] + "_input"
	...
	step_out_name := mysql_tab_infos[1] + "_output"
